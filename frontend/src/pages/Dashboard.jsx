...
import {
  ...
  Mail, Loader2, X, MapPin, User, Building2, RefreshCw
} from 'lucide-react'
...
export default function Dashboard() {
  ...
  const [gmailInterviews, setGmailInterviews] = useState([])
  const [gmailError,      setGmailError]      = useState('')
  const popupRef = useRef(null)

  // Check if Gmail is configured on mount
  useEffect(() => {
    fetch('/api/gmail/status')
      .then(r => r.json())
      .then(d => setGmailConfigured(d.configured))
      .catch(() => {})
  }, [])
  ...
  useEffect(() => {
    const handler = (event) => {
      if (event.data?.gmailInterviews !== undefined) {
        setGmailLoading(false)
        const interviews = event.data.gmailInterviews
        if (interviews.length === 0) {
          setGmailError('No upcoming interviews found in your last 60 days of email.')
        } else {
          setGmailInterviews(interviews)
          setGmailModal(true)
        }
      }
      if (event.data?.gmailError) {
        setGmailLoading(false)
        if (event.data.gmailError !== 'access_denied') {
          setGmailError(`Gmail error: ${event.data.gmailError}`)
        }
      }
    }
  ...
  const openGmailAuth = async () => {
    ...
    try {
      ...
      if (res.status === 501) {
        setGmailLoading(false)
        setGmailError('Gmail not configured — make sure the backend is restarted after adding GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET to .env')
        return
      }
      if (!res.ok) throw new Error(data.detail || 'Failed to get auth URL')
      const popup = window.open(data.url, 'gmail-auth', 'width=520,height=640,scrollbars=yes')
      popupRef.current = popup
      // If user closes popup without finishing, clear loading
      const timer = setInterval(() => {
        if (popup?.closed) { setGmailLoading(false); clearInterval(timer) }
      }, 1000)
    } catch (e) {
      setGmailLoading(false)
      setGmailError('Could not reach backend — is uvicorn running on port 8000?')
    }
  }

  const applyGmailInterview = (interview) => {
    // Persist to localStorage so countdown banner picks it up
    if (interview.role)    localStorage.setItem('ic_role', interview.role)
    if (interview.date)    localStorage.setItem('ic_interview_date', interview.date)
    if (interview.company) localStorage.setItem('ic_company', interview.company)
    setGmailModal(false)
  ...
  return (
    <PageTransition>
        ...
        <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
          ...
          <div className="flex items-start justify-between animate-fade-up">
            ...
            <div className="flex items-center gap-3">
              {/* Gmail Sync button — always visible */}
              <button
                onClick={openGmailAuth}
                disabled={gmailLoading}
                className="flex items-center gap-2 bg-white/[0.04] hover:bg-white/[0.07] border border-white/[0.08] hover:border-indigo-500/30 text-gray-300 hover:text-white text-sm font-medium px-4 py-2 rounded-xl transition-all disabled:opacity-60"
              >
                {gmailLoading
                  ? <Loader2 className="w-4 h-4 animate-spin text-indigo-400" />
                  : <Mail className="w-4 h-4 text-indigo-400" />}
                {gmailLoading ? 'Scanning…' : 'Sync Gmail'}
              </button>
          ...
          {gmailError && (
            <div className="animate-fade-up flex items-center justify-between gap-3 rounded-2xl border border-yellow-500/20 bg-yellow-500/5 px-5 py-3">
              <p className="text-xs text-yellow-400">{gmailError}</p>
              <button onClick={() => setGmailError('')} className="text-yellow-600 hover:text-yellow-400">
      ...
      {/* Gmail Interviews Modal */}
      {gmailModal && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center px-4"
          style={{ background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)' }}
          onClick={(e) => { if (e.target === e.currentTarget) setGmailModal(false) }}
        >
          <div className="bg-[#111113] border border-white/[0.08] rounded-2xl shadow-2xl w-full max-w-lg animate-fade-up">
            {/* Header */}
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-white/[0.06]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-[15px]">Interviews Found</h3>
                  <p className="text-xs text-gray-500">Select one to set your countdown</p>
                </div>
              </div>
              <button
                onClick={() => setGmailModal(false)}
                className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-600 hover:text-gray-300 hover:bg-white/[0.05] transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="px-4 py-3 space-y-2 max-h-[420px] overflow-y-auto">
              {gmailInterviews.map((iv, i) => {
                const dateObj = iv.date ? new Date(iv.date + 'T00:00:00') : null
                const daysAway = dateObj
                  ? Math.ceil((dateObj - new Date()) / 86400000)
                  : null
                return (
                  <button
                    key={i}
                    onClick={() => applyGmailInterview(iv)}
                    className="w-full text-left p-4 rounded-xl border border-white/[0.06] bg-white/[0.02] hover:bg-indigo-600/10 hover:border-indigo-500/30 transition-all group"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5">
                          {iv.company && (
                            <span className="flex items-center gap-1 text-sm font-bold text-white group-hover:text-indigo-200 truncate">
                              <Building2 className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
                              {iv.company}
                            </span>
                          )}
                          {iv.type && (
                            <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-indigo-500/15 text-indigo-400 border border-indigo-500/20 shrink-0">
                              {iv.type}
                            </span>
                          )}
                        </div>
                        <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500">
                          {iv.role && (
                            <span className="flex items-center gap-1">
                              <User className="w-3 h-3" /> {iv.role}
                            </span>
                          )}
                          {iv.interviewer && (
                            <span className="flex items-center gap-1">
                              <MapPin className="w-3 h-3" /> with {iv.interviewer}
                            </span>
                          )}
                          {iv.time && (
                            <span className="flex items-center gap-1">
                      ...
                      {dateObj && (
                        <div className="shrink-0 text-right">
                          <p className="text-sm font-bold text-white group-hover:text-indigo-200">
